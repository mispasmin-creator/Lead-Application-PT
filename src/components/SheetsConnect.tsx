...
import {
  ...
  ShieldCheck, Info, ExternalLink, Zap, Database, CheckCircle, AlertCircle, Loader2,
} from 'lucide-react';
...
export default function SheetsConnect({ syncConfig, onUpdateConfig, onTestConnection }: SheetsConnectProps) {
  ...
  const triggerTest = async () => {
    ...
    try {
      ...
      setTestResult(ok
        ? { status: 'success', message: 'Connection successful! Live sync is active and working.' }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-5 animate-fade-in pb-8">
      ...
      <div>
        ...
        <p className="text-sm mt-1" style={{ color: 'var(--text-muted)' }}>Configure Google Sheets sync and data connection</p>
      </div>

      {/* Mode selector */}
      <div className="rounded-2xl border overflow-hidden" style={{ background: 'var(--bg-card)', borderColor: 'var(--border)' }}>
        <div className="px-5 py-4 border-b flex items-center gap-2" style={{ borderColor: 'var(--border)', background: 'var(--bg-elevated)' }}>
          <Globe className="w-4 h-4 text-emerald-500" />
          <h2 className="text-sm font-semibold" style={{ color: 'var(--text)' }}>Sync Mode</h2>
        </div>

        <div className="p-5 space-y-4">
          {/* Toggle */}
          <div className="flex gap-3">
            <button onClick={() => saveConfig('local')}
              ...
              style={!isLive ? {} : { borderColor: 'var(--border)', background: 'var(--bg-elevated)' }}>
              <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${!isLive ? 'bg-amber-100 dark:bg-amber-900/30' : ''}`}
                style={isLive ? { background: 'var(--bg-elevated)' } : {}}>
                <Database className={`w-4 h-4 ${!isLive ? 'text-amber-600 dark:text-amber-400' : ''}`}
                  style={isLive ? { color: 'var(--text-subtle)' } : {}} />
              </div>
              <div>
                <p className={`text-sm font-semibold ${!isLive ? 'text-amber-700 dark:text-amber-400' : ''}`}
                  style={isLive ? { color: 'var(--text)' } : {}}>Sandbox</p>
                <p className="text-xs" style={{ color: 'var(--text-subtle)' }}>Local browser storage</p>
              </div>
              {!isLive && <Check className="w-4 h-4 text-amber-500 ml-auto" />}
            </button>

            <button onClick={() => saveConfig('live')}
              className={`flex-1 flex items-center gap-3 p-4 rounded-xl border-2 transition-all cursor-pointer text-left ${
                isLive
              ...
              style={isLive ? {} : { borderColor: 'var(--border)', background: 'var(--bg-elevated)' }}>
              <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${isLive ? 'bg-emerald-100 dark:bg-emerald-900/30' : ''}`}
                style={!isLive ? { background: 'var(--bg-elevated)' } : {}}>
                <Zap className={`w-4 h-4 ${isLive ? 'text-emerald-600 dark:text-emerald-400' : ''}`}
                  style={!isLive ? { color: 'var(--text-subtle)' } : {}} />
              </div>
              <div>
                <p className={`text-sm font-semibold ${isLive ? 'text-emerald-700 dark:text-emerald-400' : ''}`}
                  style={!isLive ? { color: 'var(--text)' } : {}}>Live Sheet</p>
                <p className="text-xs" style={{ color: 'var(--text-subtle)' }}>Real-time Google Sheets sync</p>
              </div>
            ...
            <div className="pt-4 border-t space-y-3" style={{ borderColor: 'var(--border)' }}>
              <div>
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  ...
                  <button onClick={triggerTest} disabled={isTesting}
                    ...
                    {isTesting ? <><Loader2 className="w-3.5 h-3.5 animate-spin" /> Testing…</> : 'Test Connection'}
                  </button>
                </div>
      ...
      <div className="rounded-2xl border overflow-hidden" style={{ background: 'var(--bg-card)', borderColor: 'var(--border)' }}>
        <div className="px-5 py-4 border-b flex items-center gap-2" style={{ borderColor: 'var(--border)', background: 'var(--bg-elevated)' }}>
          ...
          <h2 className="text-sm font-semibold" style={{ color: 'var(--text)' }}>Sheet Configuration</h2>
        </div>
        <div className="p-5">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              ...
              <div key={label} className="rounded-xl px-4 py-3 border" style={{ background: 'var(--bg-elevated)', borderColor: 'var(--border)' }}>
                <p className="text-[10px] font-semibold uppercase tracking-widest" style={{ color: 'var(--text-subtle)' }}>{label}</p>
                <p className="text-sm font-semibold mt-1 truncate" style={{ color: 'var(--text)' }} title={value}>{value}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Apps Script setup */}
      <div className="rounded-2xl border overflow-hidden" style={{ background: 'var(--bg-card)', borderColor: 'var(--border)' }}>
        <div className="px-5 py-4 border-b flex items-center justify-between" style={{ borderColor: 'var(--border)', background: 'var(--bg-elevated)' }}>
          <div className="flex items-center gap-2">
            <FileSpreadsheet className="w-4 h-4 text-emerald-500" />
            <h2 className="text-sm font-semibold" style={{ color: 'var(--text)' }}>Deploy Apps Script</h2>
          </div>
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {[
              ...
              { step: '3', title: 'Deploy Web App',      body: 'Click Deploy → New Deployment → Web App. Set access to Anyone. Copy the URL.' },
            ].map(({ step, title, body }) => (
              <div key={step} className="rounded-xl p-4 border" style={{ background: 'var(--bg-elevated)', borderColor: 'var(--border)' }}>
                <div className="flex items-center gap-2 mb-2">
                  <span className="w-6 h-6 rounded-lg bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400 flex items-center justify-center text-xs font-bold shrink-0">{step}</span>
